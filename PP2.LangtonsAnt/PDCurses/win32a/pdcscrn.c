...
an integral number of columns and rows doesn't work.  You can therefore
end up in a loop where the code keeps trying to resize a window that isn't
actually resizing.  So,  _when running in Wine only_,  we want that code
not to be executed... which means having to figure out:  are we running
under Wine?  Which means that when PDCurses/Win32a is initialized,  we
set the following 'wine_version' pointer.  One could actually call
wine_version(),  if not NULL,  to get the current Wine version. */
...
/*  printf( "Size was %d x %d; will be %d x %d\n",
                PDC_n_rows, PDC_n_cols, new_n_rows, new_n_cols);
*/  SP->resized = FALSE;
